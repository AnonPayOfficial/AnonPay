...
const char * const BITCOIN_CONF_FILENAME = "anonpay.conf";
const char * const BITCOIN_PID_FILENAME = "anonpayd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "anonpayd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AnonPayCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AnonPayCore
    // Mac: ~/Library/Application Support/AnonPayCore
    // Unix: ~/.anonpaycore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AnonPayCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AnonPayCore";
#else
    // Unix
    return pathRet / ".anonpaycore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty anonpay.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("AnonPay Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The AnonPay Core developers";
    }
