...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 ANPVE irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on anonpaycentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by smartnode configured in anonpay.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.pushKV("anonpay.conf", statusObj);
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by smartnode configured in anonpay.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* AnonPay features */
    { "anonpay",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "anonpay",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "anonpay",               "gobject",                &gobject,                {} },
    { "anonpay",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
