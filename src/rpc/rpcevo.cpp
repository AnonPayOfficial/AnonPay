...
std::string GetHelpString(int nParamNum, std::string strParamName)
{
    static const std::map<std::string, std::string> mapParamHelp = {
        {"collateralAddress",
            "%d. \"collateralAddress\"        (string, required) The anonpay address to send the collateral to.\n"
        },
        {"collateralAmount",
        ...
        {"ownerAddress",
            "%d. \"ownerAddress\"             (string, required) The anonpay address to use for payee updates and proposal voting.\n"
            "                              The corresponding private key does not have to be known by your wallet.\n"
            "                              The address must be unused and must differ from the collateralAddress.\n"
        },
        {"payoutAddress_register",
            "%d. \"payoutAddress\"            (string, required) The anonpay address to use for smartnode reward payments.\n"
        },
...
void protx_register_fund_help(CWallet* const pwallet)
{
    throw std::runtime_error(
            ...
            "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move the specified collateralAmount of ANP\n"
...
UniValue protx_register(const JSONRPCRequest& request)
{
    ...
    if (!request.params[paramIdx + 6].isNull()) {
        ...
        if (!IsValidDestination(fundDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid AnonPay address: ") + request.params[paramIdx + 6].get_str());
    }
...
UniValue protx_update_service(const JSONRPCRequest& request)
{
    ...
    if (!request.params[5].isNull()) {
        ...
        if (!IsValidDestination(feeSource))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid AnonPay address: ") + request.params[5].get_str());
    } else {
...
UniValue protx_revoke(const JSONRPCRequest& request)
{
    ...
    if (!request.params[4].isNull()) {
        ...
        if (!IsValidDestination(feeSourceDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid AnonPay address: ") + request.params[4].get_str());
        FundSpecialTx(pwallet, tx, ptx, feeSourceDest);
...
void protx_quick_setup_help(CWallet* const pwallet)
{
    throw std::runtime_error(
      ...
      "  \"tx\":                    (string) The raw transaction hex of this protx without signiature.\n"
			...
			"  \"anonpay.conf\" :       (string) The content of anonpay.conf to be used in vps node.\n"
      "}\n"
			"\nExamples:\n"
...
UniValue createConfigFile(string blsPrivateKey, string ip, string address) {
	...
	string fileName = get_current_dir() + "/" + address + "_anonpay.conf";
...
//register_prepare "collateralHash" collateralIndex "ipAndPort" "ownerAddress" "operatorPubKey" "votingAddress" operatorReward "payoutAddress" ( "feeSourceAddress" )
	...
	result.pushKV("anonpay.conf",config.get_str());
