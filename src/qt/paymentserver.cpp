...
const QString BITCOIN_IPC_PREFIX("anonpay:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/anonpay-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/anonpay-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/anonpay-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the anonpay: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // anonpay: URI
        {
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click anonpay: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start anonpay: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling anonpay: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in anonpay: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("anonpay://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'anonpay://' is not a valid URI. Use 'anonpay:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // anonpay: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid AnonPay address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    if (request.IsInitialized()) {
        ...
        // Make sure any payment requests ianonpayd are still valid.
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom anonpay addresses are not supported
        ...
        // AnonPay amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
