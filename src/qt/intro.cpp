    ...
    signalled(false)
{
    ...
    ui->sizeWarningLabel->setText(
        tr("%1 will download and store a copy of the AnonPay block chain.").arg(tr(PACKAGE_NAME)) + " " +
...
bool Intro::pickDataDirectory(interfaces::Node& node)
{
    ...
    if(!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) || gArgs.GetBoolArg("-choosedatadir", DEFAULT_CHOOSE_DATADIR) || dataDirDefaultCurrent != dataDirDefaultSettings)
    {
        ...
        intro.setWindowIcon(QIcon(":icons/anonpay"));
     ...
     * override -datadir in the anonpay.conf file in the default data directory
     * (to be consistent with anonpayd behavior)
     */
