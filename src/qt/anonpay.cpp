...
{
    return QCoreApplication::translate("anonpay-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in anonpay.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in anonpay.qrc)
...
/** Class encapsulating AnonPay Core startup and shutdown.
...
/** Main AnonPay application object */
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // anonpay: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. AnonPay Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
static void SetupUIArgs()
{
    ...
    gArgs.AddArg("-windowtitle=<name>", _("Sets a window title which is appended to \"AnonPay Core - \""), false, OptionsCategory::GUI);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(anonpay);
    Q_INIT_RESOURCE(anonpay_locale);
    ...
    /// 6. Determine availability of data and blocks directory and parse anonpay.conf
    ...
    // anonpay: links repeatedly have their payment requests routed to this process:
