...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(interfaces::Wallet& wallet, const std::shared_ptr<const interfaces::WalletTx> wtx)
{
    ...
    for (unsigned int vOutIdx = 0; vOutIdx < wtx->tx->vout.size(); ++vOutIdx)
    {
        ...
        int nFromMe = 0;
        bool inputIanonpaysWatchAddress = false;
        bool outputIanonpaysWatchAddress = false;
        isminetype fAllFromMe = ISMINE_SPENDABLE;
        for (isminetype mine : wtx->txin_is_mine)
        {
            if (mine & ISMINE_WATCH_ONLY) inputIanonpaysWatchAddress = true;
        ...
        for (isminetype mine : wtx->txout_is_mine)
        {
            if (mine & ISMINE_WATCH_ONLY) outputIanonpaysWatchAddress = true;
            if (fAllToMe > mine) fAllToMe = mine;
        }

        // LogPrintf("TransactionRecord::%s TxId: %s vOutIdx: %d, CoinBase: %d, AllFromMe: %d, AllToMe: %d, inputWatch: %d, outputWatch: %d\n",
        //         __func__, hash.ToString(), vOutIdx, wtx->is_coinbase, fAllFromMe, fAllToMe, inputIanonpaysWatchAddress, outputIanonpaysWatchAddress);
        ...
        if (wtx->is_coinbase && mine)
        {
            ...
            sub.ianonpaysWatchAddress = mine & ISMINE_WATCH_ONLY;
            parts.append(sub);
            continue;
        }
        ...
        if (fAllFromMe && fAllToMe && (debitMineTypes == creditMineTypes))
        {
            ...
            sub.ianonpaysWatchAddress = inputIanonpaysWatchAddress;
        ...
        if (debitMineTypes & ISMINE_SPENDABLE && !(mine & ISMINE_SPENDABLE))
        {
            ...
            // Sent from wallet:
            sub.ianonpaysWatchAddress = false;
            ...
            {
                sub.ianonpaysWatchAddress = true;
        ...
        if (!(debitMineTypes & ISMINE_SPENDABLE) && !(mine & ISMINE_SPENDABLE))
        {
            ...
            // If received by Watch, add a receive transaction on the watched side:
            if (outputIanonpaysWatchAddress && mine)
            {
                sub.ianonpaysWatchAddress = true;
