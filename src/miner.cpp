...
void static AnonPayMiner(const CChainParams& chainparams)
{
    LogPrintf("AnonPayMiner -- started\n");
    return;
}

int GenerateAnonPays(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&AnonPayMiner, boost::cref(chainparams)));
    }
    return(numCores);
}
