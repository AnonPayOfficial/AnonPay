...
#if defined(NDEBUG)
# error "AnonPay Core cannot be compiled without assertions."
#endif
 ...
 *  We have 4 times smaller block times in AnonPay, so we need to push 4 times more invs per 1MB. */
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /*
        AnonPay Related Inventory Messages
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        // ANPVE this code also handles non-TXs (Dash specific messages)
