...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf("%s anonpay-tx utility version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
            "Usage:\n"
              "  anonpay-tx [options] <hex-tx> [commands]  Update hex-encoded anonpay transaction\n" +
              "  anonpay-tx [options] -create [commands]   Create hex-encoded anonpay transaction\n" +
              "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded anonpay transaction
