...
public:
    ...
    // AnonPay Specific WalletInitInterface InitCoinJoinSettings
 ...
 * between anonpayd, and anonpay-qt and non-server tools.
*/
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-sporkaddr=<anonpayaddress>", "Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.", false, OptionsCategory::DEBUG_TEST);
    ...
    gArgs.AddArg("-platform-user=<user>", "Set the username for the \"platform user\", a restricted user intended to be used by AnonPay Platform, to the specified username.", false, OptionsCategory::SMARTNODE);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/AnonPayOfficial/AnonPay>";
    const std::string URL_WEBSITE = "<https://anonpay.net>";
...
/** Sanity checks
 *  Ensure that AnonPay Core is running in a usable environment with all
...
{
    // Make sure only a single AnonPay Core process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if AnonPay Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if AnonPay Core is started while in a temporary directory.\n",
    ...
    // ********************************************************* Step 10c: schedule AnonPay-specific tasks
