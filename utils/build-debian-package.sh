...
PACKAGE_VERSION=$($SRC_PATH/src/anonpayd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/anonpayd $DEB_BIN
cp $SRC_PATH/src/anonpay-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/anonpay-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/anonpayd.1 $DEB_MAN
cp $SRC_DOC/man/anonpay-cli.1 $DEB_MAN
cp $SRC_DOC/man/anonpay-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/anonpayd.bash-completion $DEB_CMP/anonpayd
cp $SRC_PATH/contrib/anonpay-cli.bash-completion $DEB_CMP/anonpay-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/anonpayd.1
gzip --best -n $DEB_MAN/anonpay-cli.1
gzip --best -n $DEB_MAN/anonpay-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/anonpayd $DEB_BIN/anonpay-cli
