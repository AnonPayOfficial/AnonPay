...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/AnonPayParams"
else
    PARAMS_DIR="$HOME/.anonpay-params"
fi
...
DOWNLOAD_URL="https://github.com/nvolprotocol/anonpay/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with ANP_DISABLE_SOMETHING=1
ANP_DISABLE_WGET="${ANP_DISABLE_WGET:-}"
#ANP_DISABLE_IPFS="${ANP_DISABLE_IPFS:-}"
ANP_DISABLE_CURL="${ANP_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$ANP_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$ANP_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$ANP_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the AnonPay parameters!
...
This script will fetch the AnonPay SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common AnonPay SNARK parameters. Note that it is
