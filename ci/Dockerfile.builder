...
# anonpay_hash
RUN git clone https://github.com/nvolprotocol/anonpay_hash
RUN cd anonpay_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} anonpay
RUN useradd -u ${USER_ID} -g anonpay -s /bin/bash -m -d /anonpay anonpay
...
RUN mkdir /anonpay-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /anonpay-src && \
