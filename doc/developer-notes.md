...
- [Development guidelines](#development-guidelines)
    - [General AnonPay Core](#general-anonpay-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
AnonPay Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/nvolprotocol/anonpay/blob/master/contrib/valgrind.supp))
...
$ valgrind -v --leak-check=full src/anonpayd -printtoconsole
```
...
AnonPay Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as AnonPay Core, where everyone uses
...
pay attention to for reviewers of AnonPay Core code.

General AnonPay Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: AnonPay Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against AnonPay Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `anonpay-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `anonpay-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of AnonPay Core, the wallet was always
