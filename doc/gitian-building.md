...
*Setup instructions for a Gitian build of AnonPay Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the AnonPay
...
- [Building AnonPay Core](#building-anonpay-core)
...
After Installation
-------------------
The next step in the guide ianonpays logging in as root via SSH.
...
Clone the git repositories for AnonPay Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/nvolprotocol/anonpay
...
in the AnonPay Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building AnonPay Core
----------------

To build AnonPay Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the AnonPay Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/anonpay/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/nvolprotocol/anonpay
...
For example:
```bash
URL=https://github.com/crowning-/anonpay.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit anonpay=${COMMIT} --url anonpay=${URL} ../anonpay/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit anonpay=${COMMIT} --url anonpay=${URL} ../anonpay/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit anonpay=${COMMIT} --url anonpay=${URL} ../anonpay/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the anonpay git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../anonpay/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/anonpay
SIGPATH=/some/root/path/anonpay-detached-sigs

./bin/gbuild --url anonpay=${BTCPATH},signature=${SIGPATH} ../anonpay/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/anonpay-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/anonpay-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/anonpay-osx-build.assert
```
...
[anonpay/gitian.sigs](https://github.com/nvolprotocol/gitian.sigs/) repository, or if that's not possible to create a pull
request.
