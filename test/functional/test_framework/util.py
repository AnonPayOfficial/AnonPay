...
def assert_fee_amount(fee, tx_size, fee_per_kB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s ANPVE too low! (Should be %s ANPVE)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 2) * fee_per_kB / 1000:
        raise AssertionError("Fee of %s ANPVE too high! (Should be %s ANPVE)" % (str(fee), str(target_fee)))
...
def initialize_datadir(dirname, n, chain):
    ...
    if chain == 'testnet3':
        ...
        chain_name_conf_arg_value = '1'
    with open(os.path.join(datadir, "anonpay.conf"), 'w', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "anonpay.conf")):
        with open(os.path.join(datadir, "anonpay.conf"), 'r', encoding='utf8') as f:
            for line in f:
