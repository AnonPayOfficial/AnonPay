...
"""Class for anonpayd node under test"""
...
class TestNode():
    """A class for representing a anonpayd node under test.
    ...
    def __del__(self):
        # Ensure that we don't leave any anonpayd processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, stderr=None, *args, **kwargs):
        ...
        # unclean shutdown), it will get overwritten anyway by anonpayd, and
        # potentially interfere with our attempt to authenticate
        delete_cookie_file(self.datadir, self.chain)
        self.process = subprocess.Popen(all_args, stderr=stderr, *args, **kwargs)
        self.running = True
        self.log.debug("anonpayd started, waiting for RPC to come up")

    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the anonpayd process. Returns False if unable to connect."""
        # Poll at a rate of four times per second
        poll_per_s = 4
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                raise FailedToStartError(self._node_msg(
                    'anonpayd exited with status {} during initialization'.format(self.process.returncode)))
            try:
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcassword. anonpayd still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to anonpayd")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, partial_match=False, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to anonpayd
        expected_msg: regex that stderr should match when anonpayd fails

        Will throw if anonpayd starts without an error.
        Will throw if an expected_msg is provided and it does not match anonpayd's stdout."""
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                self.log.debug('anonpayd failed to start: %s', e)
                self.running = False
                self.process = None
                ...
                if expected_msg is None:
                    assert_msg = "anonpayd should have exited with an error"
                else:
                    assert_msg = "anonpayd should have exited with expected error " + expected_msg
...
class TestNodeCLI():
    """Interface to anonpay-cli for an individual node"""
    ...
    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with anonpay-cli command-line options
    ...
    def send_cli(self, command=None, *args, **kwargs):
        """Run anonpay-cli command. Deserializes returned string as python object."""

        pos_args = [str(arg) for arg in args]
        named_args = [str(key) + "=" + str(value) for (key, value) in kwargs.items()]
        assert not (pos_args and named_args), "Cannot use positional arguments and named arguments in the same anonpay-cli call"
        ...
        self.log.debug("Running anonpay-cli command: %s" % command)
