...
class BitcoinTestFramework():
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave anonpayds and test.* datadir on exit or error")
        ...
        parser.add_option("--srcdir", dest="srcdir", default=os.path.abspath(os.path.dirname(os.path.realpath(__file__)) + "/../../../src"),
                          help="Source directory containing anonpayd/anonpay-cli (default: %default)")
        ...
        parser.add_option("--usecli", dest="usecli", default=False, action="store_true",
                          help="use anonpay-cli instead of RPC for all commands")
        parser.add_option("--anonpayd-arg", dest="dashd_extra_args", default=[], type='string', action='append',
                          help="Pass extra args to all anonpayd instances")
        ...
        self.options.bitcoind = os.getenv("BITCOIND", default=config["environment"]["BUILDDIR"] + '/src/anonpayd' + config["environment"]["EXEEXT"])
        self.options.bitcoincli = os.getenv("BITCOINCLI", default=config["environment"]["BUILDDIR"] + '/src/anonpay-cli' + config["environment"]["EXEEXT"])
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: anonpayds were not stopped and may still be running")
    ...
    def stop_node(self, i, wait=0):
        """Stop a anonpayd test node"""
        self.nodes[i].stop_node(wait=wait)
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self, wait=0):
        """Stop multiple anonpayd test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as anonpayd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self, extra_args=None, stderr=None):
        ...
        if create_cache:
            ...
            # Create cache directories, run anonpayds:
            self.set_genesis_mocktime()
            for i in range(MAX_NODES):
...
class AnonPayTestFramework(BitcoinTestFramework):
    def set_anonpay_test_params(self, num_nodes, masterodes_count, extra_args=None, fast_dip3_enforcement=False):
...
def skip_if_no_bitcoind_zmq(test_instance):
    """Skip the running test if anonpayd has not been compiled with zmq support."""
    config = configparser.ConfigParser()
    config.read_file(open(test_instance.options.configfile))

    if not config["components"].getboolean("ENABLE_ZMQ"):
        raise SkipTest("anonpayd has not been built with zmq enabled.")
