...
def main():
    ...
    # args to be passed on always start with two anonpayes; tests are the remaining unknown args
    ...
    tmpdir = "%s/anonpay_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and anonpayd must all be enabled")
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, failfast=False, runs_ci, combined_logs_len=0):
    ...
    # Warn if anonpayd is already running (unix only)
    try:
        pidof_output = subprocess.check_output(["pidof", "anonpayd"])
        if not (pidof_output is None or pidof_output == b''):
            print("%sWARNING!%s There is already a anonpayd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
...
class RPCCoverage():
    ...
    commands per `anonpay-cli help` (`rpc_interface.txt`).
