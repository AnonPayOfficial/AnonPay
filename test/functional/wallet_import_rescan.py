...
class Variant(collections.namedtuple("Variant", "call data rescan prune")):
    ...
    def check(self, txid=None, amount=None, confirmations=None):
        ...
        if txid is not None:
            ...
            # ianonpaysWatchonly will be true if either the transaction output
            # or inputs are watchonly).
            if self.data != Data.priv:
                assert_equal(address["ianonpaysWatchonly"], True)
            else:
                assert_equal("ianonpaysWatchonly" not in address, True)
    ...
    def setup_network(self):
        ...
        for i, import_node in enumerate(IMPORT_NODES, 2):
            if import_node.prune:
                # txindex is enabled by default in AnonPay and needs to be disabled for import-rescan.py
